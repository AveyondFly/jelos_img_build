...
OPACITY="1.000000"
AR_INDEX="22"
BEZELDIR="/storage/.config/bezels"
INIFILE="/storage/.config/system/bezels/settings.ini"
BEZESET="/storage/.config/system/configs/system.cfg"
BEZ_SRC_DEFAULT="/storage/roms/bezels"
BEZ_SRC_HDMI="/storage/roms/bezels_hdmi"
...
if [ ! -L $BEZELDIR ]; then
    if grep -q "720x720" "/sys/class/graphics/fb0/modes"; then
        ln -sf $BEZ_SRC_DEFAULT $BEZELDIR
    else
        ln -sf $BEZ_SRC_HDMI $BEZELDIR
    fi
fi
