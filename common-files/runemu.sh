...
if [ -f "/storage/.config/flag/fast_mode.flg" ]; then
    /usr/bin/runemu_fast.sh "$1" "$2" "$3" "$4"
    exit 0
fi
...
if [[ "${CORE}" == "auto" ]]; then
	RUNTHIS='${RUN_SHELL} "autocores" "${ROMNAME}" "${EMULATOR}" "${EMUPERF}"'
fi

sed -i '/input_player1_turbo_btn/d' /storage/.config/retroarch/retroarch.cfg
if [[ ${PLATFORM} == "varcade" ]]; then
    echo 'input_player1_turbo_btn = "3"' >> /storage/.config/retroarch/retroarch.cfg
elif [ -f /storage/.tk_a ]; then
    echo 'input_player1_turbo_btn = "1"' >> /storage/.config/retroarch/retroarch.cfg
elif [ -f /storage/.tk_b ]; then
    echo 'input_player1_turbo_btn = "0"' >> /storage/.config/retroarch/retroarch.cfg
elif [ -f /storage/.tk_x ]; then
    echo 'input_player1_turbo_btn = "2"' >> /storage/.config/retroarch/retroarch.cfg
elif [ -f /storage/.tk_y ]; then
    echo 'input_player1_turbo_btn = "3"' >> /storage/.config/retroarch/retroarch.cfg
elif [ -f /storage/.tk_l3 ]; then
    echo 'input_player1_turbo_btn = "11"' >> /storage/.config/retroarch/retroarch.cfg
elif [ -f /storage/.tk_r3 ]; then
    echo 'input_player1_turbo_btn = "12"' >> /storage/.config/retroarch/retroarch.cfg
fi
